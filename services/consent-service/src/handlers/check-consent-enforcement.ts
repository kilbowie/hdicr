...
function evaluateRequestedUsage(
  ...
  if (requestedUsage === "ai_training") {
    const allowed =
      (policy.aiControls as Record<string, unknown> | undefined)
        ?.trainingAllowed === true;
    return allowed
      ? { allowed: true }
      : {
          allowed: false,
          reason: "AI training is not permitted by actor consent settings",
        };
  }
  ...
  if (requestedUsage === "synthetic_media") {
    ...
    return allowed
      ...
      : {
          allowed: false,
          reason:
            "Synthetic media generation is not permitted by actor consent settings",
        };
  }

  const mediaUsage =
    (policy.mediaUsage as Record<string, unknown> | undefined) || {};
  ...
  const mediaPermission = keys
    .map((key) => mediaUsage[key])
...
export async function checkConsentEnforcement(
  ...
  try {
    ...
    if (licenseResult.rows.length === 0) {
      return buildResponse(403, {
        decision: "deny",
        reason:
          "No active license found for this actor. Request license first.",
        error: "NO_LICENSE",
      });
    }
    ...
    const evaluation = evaluateRequestedUsage(
      consentEntry.policy || {},
      requestedUsage,
    );
    ...
    const commercial = (consentEntry.policy?.commercial || {}) as Record<
      string,
      unknown
    >;
    ...
    const constraints = (consentEntry.policy?.constraints || {}) as Record<
      string,
      unknown
    >;
    ...
    console.error("[CONSENT] Enforcement check error", {
      tenantId,
      errorMessage,
    });
